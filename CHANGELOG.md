...
PoC 어장 범위를 변 길이 절반(면적 1/4)으로 축소. 첫 사업 PoC에 현실적인 운용 범위로 조정.

### 변경
- `data_pipeline/fish_grid.py` `DEFAULT_ZONES` bbox 축소 (중심점 유지):
  - 욕지: 25.5×31.8km → **12.8×16km** (격자 26×32 → 13×16)
  - 미조: 25.5×27.3km → **12.8×13.6km** (격자 26×27 → 13×14)
  - 매물: 25.5×27.3km → **12.8×13.6km** (격자 26×27 → 13×14)
- 데모 드론·AIS 데이터를 새 bbox 기준으로 재생성.

### 영향
- 격자 수가 1/4로 줄어 핫스팟 점수 분해능이 그만큼 세분화됨.
- 1대 드론이 어장 한 바퀴 도는 시간 ≈절반 → 시뮬레이션 시연 시각적으로 더 빠르게 진행.
- 첫 사업 PoC 운용 비용·시간 모두 1/4 수준으로 합리화.
